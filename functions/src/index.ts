...
const T = new Twit({
  ...
  timeout_ms:
      60 * 1000,    // optional HTTP request timeout to apply to all requests.
...
function Tweet(b64content, title, id) {
  ...
  T.post(
      'media/upload', {media_data: b64content},
      function(err, res_data, response) {
        ...
        T.post(
            'media/metadata/create', meta_params,
...
app.post('/creations', async (req, res) => {
  ...
  try {
    ...
    const exists = await pgPool.query(
        'SELECT exists( SELECT 1 FROM creations WHERE id = $1 )', [publicId]);
    ...
    function uploadPNG(pngData, suffix) {
      const mimeType =
          pngData.match(/data:([a-zA-Z0-9]+\/[a-zA-Z0-9-.+]+).*,.*/)[1];
      const filename = `creations/${id}${suffix}`;
      const base64EncodedImageString =
          pngData.replace(/^data:image\/\w+;base64,/, '');
    ...
    await Promise.all(
        [uploadPNG(cells, '.data.png'), uploadPNG(image, '.png')]);
...
app.get('/creations/:id', async (req, res) => {
  ...
  try {
    const get =
        await pgPool.query('SELECT *  FROM creations WHERE id = $1', [id]);
    ...
    if (get.rowCount > 0) {
      ...
      res.status(404).json(
          {errorCode: 404, errorMessage: `submission '${id}' not found`});
    }
  } catch (error) {
